...
<html>
  <head>
    ...
    <link rel="stylesheet" href="../<?php echo $customer;?>/css/custom.css?v=1.22">
  ...
  <body>
    ...
    $query = 'SELECT catid, nom, visible FROM categorie WHERE customid = ' . $customid;
		...
		if ($result = $conn->query($query)) {
    		while ($row = $result->fetch_row()) {
    			if ($row[2] > 0 )
    			{
              	...
              	if ($row2[6]>0)
              	  echo '<img class="rightpic" src="../' . $customer . '/upload/' . $row2[5] . '" alt = "nopic">';
              	echo '<a class="nom">';
       	      	echo $row2[1];
       	      	echo '<br />';
       	      	echo '</a>';
    	...
    	$query3 = 'SELECT artid, nom, prix, unite, description, image, imgvisible FROM article WHERE customid = ' . $customid . ' AND visible = 1 AND obligatoire = 1';
        	...
        	if ($row3[6]>0)
        	  echo '<img class="rightpic" src="../' . $customer . '/upload/' . $row3[5] . '" alt = "nopic">';
        	echo '<a class="nom">';
 	      	echo $row3[1];
 	      	echo '<br />';
 	      	echo '</a>';
