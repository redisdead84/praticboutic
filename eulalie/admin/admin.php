...
<html id="backhtml">
  ...
  <?php
    ...
    if (empty($_SESSION['auth']) == TRUE)
    {
   	  header("LOCATION: index.php");
   	  exit();
    }	
    
    if (strcmp($_SESSION['auth'],'oui') != 0)
	  {
   	  header("LOCATION: index.php");
   	  exit();
	  }
    ...
    if (strcmp($_SESSION['customer'], $customer) != 0)
	  {
   	  header("LOCATION: index.php");
   	  exit();
	  }
    ...
    if ($conn->connect_error) 
    {
      ...
      if ($cmpi == 0)
      { 
      	$q = ' INSERT INTO categorie (nom, visible) ';
    	  $q = $q . 'VALUES ("' . htmlentities($_POST['inpnom']) . '" ,"' . $bVis . '")';
      ...
      {
        $q = 'UPDATE categorie ';
        $q = $q . 'SET nom = "' . htmlentities($_POST['inpnom']) . '",visible = "' . $bVis . '" ';
      ...
      $fichier = isset($_FILES['inpimgart']) ? basename($_FILES['inpimgart']['name']) : '';  
	 	  ...
	 	  {  
        $dossier = '../upload/';
        $taille_maxi = intval(GetValeurParam("Max_file_size", $conn));
      ...
      if(empty($error) == TRUE)
      {
        if ($cmpi == 0)
        {
          $q = ' INSERT INTO article (nom, prix, description, catid, unite, visible, image, imgvisible, obligatoire) ';
      	  $q = $q . 'VALUES ("' . htmlentities($_POST['inpnomart']) . '","' . $_POST['inpprixart'] . '",
        ...
        if ($cmpu == 0)
        {
          ...
          {
            $q = 'UPDATE article ';
            $q = $q . 'SET nom = "' . htmlentities($_POST['inpnomart']) . '", prix = "' . $_POST['inpprixart'] . '",
      ...
      if ($cmpi == 0)
      { 
      	$q = ' INSERT INTO relgrpoptart (grpoptid, artid, visible) ';
    	  $q = $q . 'VALUES ("' . $_POST['inpgrpopt'] . '", "' . $_POST['inpart'] . '", "' . $bVis . '")';
        if ($conn->query($q) === FALSE) 
        {
  		    $ins = 1;
  		    $error = $conn->error;
      ...
      if ($cmpu == 0)
      {
        ...
        $q = $q . 'SET grpoptid = "' . $_POST['inpgrpopt'] . '", artid = "' . $_POST['inpart'] . '", visible = "' . $bVis . '" ';
      ...
      if ($cmpi == 0)
      { 
      	$q = 'INSERT INTO groupeopt (nom, visible, multiple) ';
    	  $q = $q . 'VALUES ("' . htmlentities($_POST['inpnom']) . '", "' . $bVis . '", "' . $bMulti . '")';
        if ($conn->query($q) === FALSE) 
        {
  		    $ins = 1;
  		    $error = $conn->error;
      ...
      {
        $q = 'UPDATE groupeopt ';
        $q = $q . 'SET nom = "' . htmlentities($_POST['inpnom']) . '", visible = "' . $bVis . '", multiple = "' . $bMulti . '" ';
      ...
      if ($cmpi == 0)
      { 
      	$q = 'INSERT INTO `option` (nom, surcout, grpoptid, visible) ';
    	  $q = $q . 'VALUES ("' . htmlentities($_POST['inpnom']) . '","' . $_POST['inpsurcout'] . '","' . $_POST['inpgrpopt'] . '", "' . $bVis . '")';
        if ($conn->query($q) === FALSE) 
        {
  		    $ins = 1;
  		    $error = $conn->error;
      ...
      {
        $q = 'UPDATE `option` ';
        $q = $q . 'SET nom = "' . htmlentities($_POST['inpnom']) . '", surcout = "' . $_POST['inpsurcout'] . '", grpoptid = "' . $_POST['inpgrpopt'] . '", visible = "' . $bVis . '" ';
      ...
      if ($cmpi == 0)
      {
        $q = ' INSERT INTO administrateur (pseudo, pass, email, actif) ';
    	  $q = $q . 'VALUES ("' . htmlentities($_POST['inppseudoad']) . '","' . password_hash($_POST['inppassad'], PASSWORD_DEFAULT) . '",
      ...
      if ($cmpu == 0)
      {
        ...
        if (strcmp($motdepasse, "") == 0)
        {
          ...
          $q = $q . 'SET pseudo = "' . htmlentities($_POST['inppseudoad']) . '", email = "' . $_POST['inpemailad'] . '", actif = "' . $bAct . '" ';
      	  $q = $q . 'WHERE adminid = "' . $recupd . '"';
        }
        else 
        {
          $q = 'UPDATE administrateur ';
          $q = $q . 'SET pseudo = "' . htmlentities($_POST['inppseudoad']) . '", pass = "' . password_hash($motdepasse, PASSWORD_DEFAULT) . '",
      ...
      if ($cmpi == 0)
      { 
      	$q = ' INSERT INTO parametre (nom, valeur, commentaire) ';
    	  $q = $q . 'VALUES ("' . $_POST['inpnompa'] . '" ,"' . $_POST['inpvaleurpa'] . '" ,"' . $_POST['inpcommentpa'] . '")';
        if ($conn->query($q) === FALSE) 
        {
  		    $ins = 1;
  		    $error = $conn->error;
      ...
      {
        $q = 'UPDATE parametre ';
        $q = $q . 'SET nom = "' . $_POST['inpnompa'] . '",valeur = "' . $_POST['inpvaleurpa'] . '",commentaire = "' . $_POST['inpcommentpa'] . '" ';
      ...
      if ($cmpi == 0)
      { 
      	$q = 'INSERT INTO `cpzone` (codepostal, ville, actif) ';
    	  $q = $q . 'VALUES ("' . $_POST['inpcp'] . '","' . htmlentities($_POST['inpville']) . '","' . $bActif . '")';
        if ($conn->query($q) === FALSE) 
        {
  		    $ins = 1;
  		    $error = $conn->error;
      ...
      {
        $q = 'UPDATE `cpzone` ';
        $q = $q . 'SET codepostal = "' . $_POST['inpcp'] . '", ville = "' . htmlentities($_POST['inpville']) . '", actif = "' . $bActif . '" ';
    ...
    else if (strcmp($mode, 'avance') == 0)
    {
	  ...
	  echo ' ! <br />';
	  echo '<a href="logout.php"><button type="button">Deconnexion</button></a>'," \n ";
    if (strcmp($_SESSION['mode'],'basique')== 0)
    {
      echo '<a href="admin.php?mode=avance#tabRgoa"> <input type="button" value="Mode Avancé"> </a>';
    }
    else if (strcmp($_SESSION['mode'],'avance')== 0)  
    {
      echo '<a href="admin.php?mode=basique#tabCat"> <input type="button" value="Mode Basique"> </a>';
    }
    ...
    if (empty($modif) && empty($ins))
    {
      ...
      echo '<th>Visible</th>';
      echo '</tr>'," \n ";
      $query2 = 'SELECT catid, nom, visible FROM categorie'  ;
      if ($result2 = $conn->query($query2)) 
	    {	
    	  ...
    	  {	
       	  ...
       	  echo '</td>';
       	  echo '<td>';
		      echo '<a href="admin.php?modifier=' . $row2[0] . '&table=categorie#tabCat"> <input type="button" value="Modifier"> </a>';
       	  echo '</td>';
      	  echo '</tr>'," \n ";
      ...
      echo '</table>'," \n ";
	    echo '<a href="admin.php?inserer=1&table=categorie#tabCat"> <input type="button" value="Insérer"> </a>'," \n ";
      echo '</div>'," \n ";      	
	  }
  	...
  	{ 
	    ...
	    if (!empty($modif))
	    {
	      $action  = 'admin.php?commande=update&table=categorie&rectou=' . $modif . '#tabCat';
		    ...
		    echo '<a href="admin.php#tabCat"><button type="button">Cancel</button></a>'," \n ";
    ...
    if (empty($modif) && empty($ins))
    {
      ...
      $query2 = 'SELECT artid, article.nom, categorie.nom as catnom, article.prix,
                 ...
                 FROM article, categorie WHERE article.catid = categorie.catid';
      if ($result2 = $conn->query($query2)) 
      {	
	      while ($row2 = $result2->fetch_row()) 
    	  {	
       	  ...
       	  echo '</td>';
       	  echo '<td>';
		      echo '<a href="admin.php?modifier=' . $row2[0] . '&table=article#tabArt"> <input type="button" value="Modifier"> </a>';
       	  echo '</td>';
      	  echo '</tr>'," \n ";
		    }  						
	  	  $result2->close();
      ...
      echo '</table>'," \n ";
	    echo '<a href="admin.php?inserer=1&table=article#tabArt"> <input type="button" value="Insérer"> </a>'," \n ";
      echo '</div>'," \n ";      	
	  }
	  ...
	  { 
	    ...
	    if (!empty($modif))
	    {
	  	  ...
	  	  $queryopt2 = 'SELECT catid, nom, visible FROM categorie';
        ...
        echo '<br />'," \n ";
	      ...
	      echo '<a href="admin.php#tabArt"><button type="button">Cancel</button></a>'," \n ";
      	echo '<input type="submit" value="Submit">'," \n ";
      	echo '</form>'," \n ";
	    }
    ...
    if (strcmp($_SESSION['mode'],'avance')== 0)  
    {
      ...
      if (empty($modif) && empty($ins))
      {
        ...
        $query2 = $query2 . 'WHERE relgrpoptart.grpoptid = groupeopt.grpoptid AND relgrpoptart.artid = article.artid';
        //echo $query2;
        if ($result2 = $conn->query($query2)) 
  	    {	
  	      while ($row2 = $result2->fetch_row()) 
      	  {	
         	  ...
         	  echo '</td>';
         	  echo '<td>';
  		      echo '<a href="admin.php?modifier=' . $row2[0] . '&table=relgrpoptart#tabRgoa"> <input type="button" value="Modifier"> </a>';
         	  echo '</td>';
        	  echo '</tr>'," \n ";
        ...
        echo '</table>'," \n ";
  	    echo '<a href="admin.php?inserer=1&table=relgrpoptart#tabRgoa"> <input type="button" value="Insérer"> </a>'," \n ";
        echo '</div>'," \n ";      	
  	  }
    	...
    	{ 
  	    ...
  	    if (!empty($modif))
  	    {
  	      $action  = 'admin.php?commande=update&table=relgrpoptart&rectou=' . $modif . '#tabRgoa';
  	  	  ...
  	  	  $queryopt2 = 'SELECT grpoptid, nom, visible FROM groupeopt'  ;
        	if ($resultopt2 = $conn->query($queryopt2)) 
  	  	  {	
  	        while ($rowopt2 = $resultopt2->fetch_row()) 
      	    {	
          ...
          echo '</select><br>'," \n ";
  	  	  ...
  	  	  $queryopt2 = 'SELECT artid, nom, visible FROM article'  ;
        	if ($resultopt2 = $conn->query($queryopt2)) 
  	  	  {	
  	        while ($rowopt2 = $resultopt2->fetch_row()) 
      	    {	
          ...
          echo '</select><br>'," \n ";
  		    ...
  		    echo '<a href="admin.php#tabRgoa"><button type="button">Cancel</button></a>'," \n ";
        	echo '<input type="submit" value="Submit">'," \n ";
        	echo '</form>'," \n ";
      ...
      if (empty($modif) && empty($ins))
      {
        ...
        echo '<th>Multiple</th>';
        echo '</tr>'," \n ";
        $query2 = 'SELECT grpoptid, nom, visible, multiple FROM groupeopt'  ;
        if ($result2 = $conn->query($query2)) 
  	    {	
  	      while ($row2 = $result2->fetch_row()) 
      	  {	
         	  ...
         	  echo '</td>';
         	  echo '<td>';
  		      echo '<a href="admin.php?modifier=' . $row2[0] . '&table=groupeopt#tabGrpo"> <input type="button" value="Modifier"> </a>';
         	  echo '</td>';
        	  echo '</tr>'," \n ";
        ...
        echo '</table>'," \n ";
  	    echo '<a href="admin.php?inserer=1&table=groupeopt#tabGrpo"> <input type="button" value="Insérer"> </a>'," \n ";
        echo '</div>'," \n ";      	
  	  }
    	...
    	{ 
  	    ...
  	    if (!empty($modif))
  	    {
  	      $action  = 'admin.php?commande=update&table=groupeopt&rectou=' . $modif . '#tabGrpo';
  		    ...
  		    echo '<a href="admin.php#tabGrpo"><button type="button">Cancel</button></a>'," \n ";
        	echo '<input type="submit" value="Submit">'," \n ";
        	echo '</form>'," \n ";
      ...
      if (empty($modif) && empty($ins))
      {
        ...
        $query2 = 'SELECT optid, `option`.nom as optnom, surcout, groupeopt.nom as grponom, 
                   `option`.visible    
                   FROM `option`, groupeopt WHERE `option`.grpoptid = groupeopt.grpoptid';
        if ($result2 = $conn->query($query2)) 
        {	
  	      while ($row2 = $result2->fetch_row()) 
      	  {	
         	  ...
         	  echo '</td>';
         	  echo '<td>';
  		      echo '<a href="admin.php?modifier=' . $row2[0] . '&table=option#tabOpt"> <input type="button" value="Modifier"> </a>';
         	  echo '</td>';
        	  echo '</tr>'," \n ";
  		    }  						
  	  	  $result2->close();
        ...
        echo '</table>'," \n ";
  	    echo '<a href="admin.php?inserer=1&table=option#tabOpt"> <input type="button" value="Insérer"> </a>'," \n ";
        echo '</div>'," \n ";      	
  	  }
  	  ...
  	  { 
          ...
          echo '<br />'," \n ";
  	      ...
  	      echo '<a href="admin.php#tabOpt"><button type="button">Cancel</button></a>'," \n ";
        	echo '<input type="submit" value="Submit">'," \n ";
        	echo '</form>'," \n ";
  	    }
      ...
      if (empty($modif) && empty($ins))
      {
        ...
        echo '<th>Actif</th>';
        echo '</tr>'," \n ";
        $query3 = 'SELECT adminid, pseudo, pass, email, actif FROM administrateur';
        ...
        {	
      	  ...
      	  {	
         	  ...
         	  echo '</td>';
         	  echo '<td>';
  		      echo '<a href="admin.php?modifier=' . $row3[0] . '&table=administrateur#tabAdmin"> <input type="button" value="Modifier"> </a>';
         	  echo '</td>';
        	  echo '</tr>'," \n ";
        	}						
  	  		$result3->close();
      	...
      	echo '</table>'," \n ";
  	  	echo '<a href="admin.php?inserer=1&table=administrateur#tabAdmin"> <input type="button" value="Insérer"> </a>'," \n ";
      	echo '</div>'," \n ";      	
  		}
          ...
          echo '<label for="inpactifad">Actif : </label><br>'," \n ";
  	      ...
  	      echo '<a href="admin.php#tabAdmin"><button type="button">Cancel</button></a>'," \n ";
        	echo '<input type="submit" value="Submit">'," \n ";
        	echo '</form>'," \n ";
  	    }
      ...
      if (empty($modif) && empty($ins))
      {
        ...
        echo '</tr>'," \n ";
        $query4 = 'SELECT paramid, nom, valeur, commentaire FROM parametre'  ;
        if ($result4 = $conn->query($query4)) 
          ...
          while ($row4 = $result4->fetch_row()) 
      	  {	
         	  ...
         	  echo '</td>';
         	  echo '<td>';
  		      echo '<a href="admin.php?modifier=' . $row4[0] . '&table=parametre#tabParam"> <input type="button" value="Modifier"> </a>';
         	  echo '</td>';
        	  echo '</tr>'," \n ";
        ...
        echo '</table>'," \n ";
  	    echo '<a href="admin.php?inserer=1&table=parametre#tabParam"> <input type="button" value="Insérer"> </a>'," \n ";
        echo '</div>'," \n ";      	
  	  }
    	...
    	{ 
  	    ...
  	    if (!empty($modif))
  	    {
  	      $action41  = 'admin.php?commande=update&table=parametre&rectou=' . $modif . '#tabParam';
  		    ...
  		    echo '<a href="admin.php#tabParam"><button type="button">Cancel</button></a>'," \n ";
        	echo '<input type="submit" value="Submit">'," \n ";
        	echo '</form>'," \n ";
  	    }
  	    if (!(empty($ins)))
      ...
      if (empty($modif) && empty($ins))
      {
        ...
        echo '<th>Actif</th>';
        echo '</tr>'," \n ";
        $query2 = 'SELECT cpzoneid, codepostal, ville, actif FROM cpzone';
        ...
        {	
          while ($row2 = $result2->fetch_row()) 
      	  {	
         	  ...
         	  echo '</td>';
         	  echo '<td>';
    	      echo '<a href="admin.php?modifier=' . $row2[0] . '&table=cpzone#tabZone"> <input type="button" value="Modifier"> </a>';
         	  echo '</td>';
        	  echo '</tr>'," \n ";
        ...
        echo '</table>'," \n ";
        echo '<a href="admin.php?inserer=1&table=cpzone#tabZone"> <input type="button" value="Insérer"> </a>'," \n ";
        echo '</div>'," \n ";      	
      }
      ...
      if ((!(empty($modif) && empty($ins))) && $cmpcpzone == 0)
        ...
        if (!empty($modif))
        {
          $action  = 'admin.php?commande=update&table=cpzone&rectou=' . $modif . '#tabZone';
    	    ...
    	    echo '<a href="admin.php#tabZone"><button type="button">Cancel</button></a>'," \n ";
        	echo '<input type="submit" value="Submit">'," \n ";
        	echo '</form>'," \n ";
